...
export const validateLowerUpperBounds = (lower: number, upper: number, numeric_tolerance = 1e-5) => {
    if (!(lower + numeric_tolerance < upper)) {
        throw new Error(`Lower bound '${lower}' can not be larger than upper bound '${upper}'.`);
    }
    return true;
};
