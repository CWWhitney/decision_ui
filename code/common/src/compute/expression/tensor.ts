...
export async function tensorToComputationValue(
  tensor: tf.Tensor,
  ...
  switch (shape.length) {
    ...
    case 2: {
      ...
      for (let r = 0; r < rows; r++) {
        series.push(data.slice(r * cols, (r + 1) * cols));
      }
      return {
...
export const getTensorForEstimateNode = (
  node: Node,
  context: ComputationContext,
): tf.Tensor => {
  if (node.type != ESTIMATE_NODE_TYPE) {
    throw new Error(
      `cannot calculate estimate node tensor for node of type '${node.type}'`,
    );
  }
  ...
  if (node.options.distribution == "deterministic") {
    ...
    const { mean, stddev } = getNormalDistributionParameter(
      node.options.lower,
      node.options.upper,
    );
  ...
  throw new Error(
    `distribution '${node.options.distribution}' tensor calculation not implemented`,
  );
};
...
export const getTensorForOperationNode = (
  ...
  evaluateExpressionForTensor: (
    expression: string,
    expressionContext: ExpressionTensorContext,
  ) => tf.Tensor,
  self: (nodeId: string) => tf.Tensor,
): tf.Tensor => {
  if (node.type != OPERATION_NODE_TYPE) {
    throw new Error(
      `cannot calculate operation node tensor for node of type '${node.type}'`,
    );
  }
  ...
  for (const variable of getVariableDependencies(node.id)) {
    expressionContext.tensorByVariable[variable] = self(
      getNodeIdForVariable(variable),
    );
  }

  return evaluateExpressionForTensor(
