...
export const useModelStore = defineStore("model", {
    ...
    getters: {
        validationErrorCount(state): number {
            return (
                Object.values(state.validation.graphs).reduce(
                    (s, g) => s + g.errors.filter(e => e.type == "error").length,
                    0
                ) +
                Object.values(state.validation.nodes).reduce(
                    (s, n) => s + n.errors.filter(e => e.type == "error").length,
                    0
                )
            );
        },
        validationInfoCount(state): number {
            return (
                Object.values(state.validation.graphs).reduce(
                    (s, g) => s + g.errors.filter(e => e.type == "info").length,
                    0
                ) +
                Object.values(state.validation.nodes).reduce(
                    (s, n) => s + n.errors.filter(e => e.type == "info").length,
                    0
                )
            );
        },
